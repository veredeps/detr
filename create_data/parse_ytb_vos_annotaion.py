...
class Instance_pil(object):
    ...
    def getInstancePixels(self, imgNp, instLabel):
        return (np.array(imgNp) == instLabel).sum()
...
class Gray2Ann(object):
    def __init__(self, pil_image, json_ann):
        self.palette = np.array(pil_image.getpalette()).reshape(-1, 3)
        palette_ids = []
        ...
        palette_len = len(palette_ids)

        self.palette = self.palette[palette_ids, :]
        if self.palette.size > 0 :
            self.gray = cv2.cvtColor(self.palette.reshape(1,-1,3).astype(np.uint8), cv2.COLOR_RGB2GRAY)
            if self.gray.size == 1:
                self.gray2Id = {self.gray.flatten()[0]: palette_ids[0]}
            else:
                self.gray = self.gray.squeeze()
                self.gray2Id = dict(zip(self.gray, palette_ids))

            if 0 not in self.gray2Id:
                self.gray2Id[0] = 0

            ann_list = []
            self.id2ann = {}
            for i in palette_ids:
                #ann_list.append(json_ann['objects'][str(i)]['category'])
                if i == 0:
                    self.id2ann[0] = 'background'
                else:
                    if str(i) in json_ann['objects']:
                        self.id2ann[i] = json_ann['objects'][str(i)]['category']
                    else:
                        self.id2ann[i] = 'unknown'
        #self.id2ann = dict(zip(self.gray, ann_list))

    def get_ann(self, gray):
        return self.id2ann[self.gray2Id[gray]]


def convert_ytb_vos(data_dir, out_dir):
    sets = ['train']
    ann_dirs = ['train/Annotations/']
    json_name = 'instances_class_%s.json'
    num_obj = 0
    num_ann = 0
    for data_set, ann_dir in zip(sets, ann_dirs):
        ...
        for vid, video in enumerate(json_ann['videos']):
            ...
            #first_frame_name = join(video, frames[0])
            #pil_img = Image.open(os.path.join(ann_dir, first_frame_name+'.png'))
            #gray2ann = Gray2Ann(pil_img, v)
            ...
            for frame in frames:
                ...
                for objId in objects:
                    ...
                    ann['category'] = gray2ann.get_ann(objId)
